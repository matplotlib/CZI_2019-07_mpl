...
streaming data and some of the design choices about data structures and
name space organization are becoming constraints as
Matplotlib is adopted by more domains.
...
top-level functions take in data, style, and \texttt{Axes}(s) and return \texttt{Artist}s.
These functions will use consistent naming and call conventions,
as discussed in \ref{sec:api_hom},  consume the data objects discussed in section
\ref{sec:dm}, and return the \texttt{Artist}s discussed in section
\ref{sec:artists}.
From the large pool of plotting functions, augmented by downstream
libraries, we can curate domain-specific name spaces to enable users
to discover the functionality they need.
...
exploration, streaming, and animation.
